...
class PitchObj(object):
    ...
    def edges_finder(self, values):
        ...
        # np.where() makes arrays match up (else warning)
        index = index[np.where(edges > 0)]
        return index.tolist()
            ...
            if np.amin(samp_values) > 0:
                ...
                try:
                    for frame in voiced_frames:
                        ...
                        # Déclenche un bug car up_interval est vide
                        # Ne detecte pas de voiced_frame dans l'enregistrement
                        tot_interval = np.arange(up_interval[0]-(self.frame_jump/2), up_interval[-1]+(self.frame_jump/2))

                        # used to be "and len(frame) > 1" which caused index out of bound errors in pchip()
                        if interp_tech is 'pchip' and len(frame) > 2:
                            up_version[tot_interval.astype(int)] = pchip(up_interval, samp_values[frame])(tot_interval)

                        elif interp_tech is 'spline' and len(frame) > 1:
                            tck, u_original = splprep([up_interval, samp_values[frame]],
                                                   u=up_interval)
                            up_version[tot_interval.astype(int)] = splev(tot_interval, tck)[1]

                        elif len(frame) == 1:
                            up_version[tot_interval.astype(int)] = samp_values[frame]
...
def yaapt(signal, **kwargs):
    ...
    pitch._studied_pitch = spec_pitch
